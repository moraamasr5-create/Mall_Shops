...
Before claiming **VS1 Complete** (see [RELEASE_GATES.md](./RELEASE_GATES.md)), starting DB Role Hardening, opening RC1 / Pilot, tagging v1.0.0, or opening a new production Module:

- [ ] **RG-001 / OP-001:** Live Cross-Tenant gate **CLOSED** (`docs/evidence/cross-tenant-latest.md` was **PASS**, then Decision Log advanced).
- [ ] Remaining Release Gates for the intended milestone are tracked in [RELEASE_GATES.md](./RELEASE_GATES.md).

DB Role Hardening must not begin until OP-001 is on the **CLOSED** path. Prefer closing existing gates over new Features/Modules.
